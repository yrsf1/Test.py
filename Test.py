...
def lagrange(xs, ys, x):
    ...
    steps = []
    for i in range(n):
        num = np.prod([(x - xs[j]) for j in range(n) if j != i])
        den = np.prod([(xs[i] - xs[j]) for j in range(n) if j != i])
        Li = num / den
        term = ys[i] * Li
        result += term
...
if st.button("⚡ احسب", use_container_width=True, type="primary"):
    ...
    with st.expander("📝 خطوات الحل", expanded=True):
        ...
        st.markdown(f"**P({x_val:g}) = {' + '.join(f'{s[\"term\"]:.4g}' for s in steps)} = `{result:.6g}`**")

    # الرسم
    mg = (max(xs) - min(xs)) * 0.35 or 1
    x_plot = np.linspace(min(xs) - mg, max(xs) + mg, 400)
    y_plot = [lagrange(xs, ys, x)[0] for x in x_plot]

    fig, ax = plt.subplots(figsize=(8, 4))
    fig.patch.set_facecolor("#0e1117")
    ax.set_facecolor("#0e1117")
    ax.plot(x_plot, y_plot, color="#38bdf8", linewidth=2, label="P(x)")
    ax.scatter(xs, ys, color="#818cf8", s=80, zorder=5, label="نقاط البيانات")
    ax.scatter([x_val], [result], color="#34d399", s=120, marker="*", zorder=6, label=f"f({x_val:g})={result:.4g}")
